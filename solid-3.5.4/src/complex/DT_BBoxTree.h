...
bool rayCast(const DT_BBoxTree& a, const DT_RootData<Shape>& rd,
              const MT_Point3& source, const MT_Point3& target, const MT_Scalar& margin, 
              MT_Scalar& lambda, MT_Vector3& normal) 
{
    ...
    { 
        return ray_cast(rd, a.m_index, source, target, margin, lambda, normal); 
    }
    else 
    {
        ...
        bool lresult = rayCast(ltree, rd, source, target, margin, lambda, normal);
        bool rresult = rayCast(rtree, rd, source, target, margin, lambda, normal);
