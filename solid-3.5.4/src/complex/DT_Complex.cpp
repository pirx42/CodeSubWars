...
inline bool ray_cast(const DT_RootData<const DT_Convex *>& rd, DT_Index index, const MT_Point3& source, const MT_Point3& target, const MT_Scalar& margin, 
                     MT_Scalar& lambda, MT_Vector3& normal)
{
    return rd.m_leaves[index]->ray_cast(source, target, margin, lambda, normal);
}

bool DT_Complex::ray_cast(const MT_Point3& source, const MT_Point3& target, const MT_Scalar& margin,
                          MT_Scalar& lambda, MT_Vector3& normal) const 
{
    DT_RootData<const DT_Convex *> rd(m_nodes, m_leaves);

    return rayCast(DT_BBoxTree(m_cbox, 0, m_type), rd, source, target, margin, lambda, normal);
}
